...
public:
  ...
  bool operator<(const lattice_point &p) const { return x_ < p.x_ or (x_ == p.x_ and y_ < p.y_); }

  lattice_point operator+(lattice_point p) const { return lattice_point(x_ + p.x_, y_ + p.y_); }
  lattice_point operator-(lattice_point p) const { return lattice_point(x_ - p.x_, y_ - p.y_); }
